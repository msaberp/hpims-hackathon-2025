"""
AIR-MS Connection Helper (Version 2 - SAP HANA Compatible)

Simplified PDC calculation that works with SAP HANA's window function restrictions.
Uses a simpler algorithm that avoids window functions in CASE statements.
"""
...
class AdherenceAnalyzer:
    ...
    def calculate_pdc_server_side(
        ...
        """
        Calculate PDC using server-side SQL - SIMPLIFIED for SAP HANA compatibility

        This version uses a simpler algorithm that works within SAP HANA's restrictions.
        It calculates PDC by:
        1. Getting all drug exposures with calculated end dates
        2. Finding gaps between consecutive fills
        3. Calculating coverage days and treatment duration
        4. Computing PDC per patient-drug
        ...
        query = f"""
        WITH drug_exposures_with_dates AS (
            SELECT
                de.person_id,
                de.drug_concept_id,
                de.drug_exposure_id,
        ...
        -- Add previous end date using LAG
        ...
        -- Join with concept table for drug names
        SELECT
            pdc.person_id,
            pdc.drug_concept_id,
